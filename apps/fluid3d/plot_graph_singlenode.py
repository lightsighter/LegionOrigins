...
def plot(cpus, speedups, label, shape, color):
    plt.plot(cpus, speedups, label = label, linestyle = 'dashed', markersize = 7, marker = shape, color=color, linewidth=0.5)
...
if __name__ == '__main__':
    ...
    plt.grid(True)
    index = 1
    for machine, framework, shape, color, data in small_plot:
        cpus, times = zip(*data)
        plot(cpus, baseline[machine] / np.array(times),
             '%s %s' % (framework, machine), shape, color)
        index += 1
    plt.legend(loc=4)
    plt.axis([0, 18, 0, 7])
