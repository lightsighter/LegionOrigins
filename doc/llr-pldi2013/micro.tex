...
For our experiment, there are 8 reduction tasks created for each node, one per processor.  Each task performs 4M reductions
...
reduction messages is scalable, but the need to send each reduction separately and the per-message overhead make it nearly 1000x slower than the reduction instance version that is
