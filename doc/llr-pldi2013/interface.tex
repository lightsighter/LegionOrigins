...
element data is stored in on or more {\tt RegionInstance} objects (line 59), each of which can be
created in an arbitrary {\tt Memory} (line 54).
...
their own completion event as a prerequisite for the consumer task.
defines a class of
...
predicated on an event and return an event corresponding to completion.  When a copy operation is
