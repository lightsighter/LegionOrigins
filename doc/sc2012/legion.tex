...
the penalty for mistakes in the placement of data or computation is very poor performance.  Thus,
to achieve high performance it is crucial that the programmer
...
operating on disjoint data, and therefore not requiring communication and able to be placed in possibly distant parts of the machine).  Most contemporary
...
others) and {\em coherence} (e.g., {\em exclusive access} and {\em
  atomic access}, among others).  Priveleges express how a task may use
...
access the same region with {\em read-only} privileges the two tasks
...
region $r$ with {\em exclusive} access, then Legion guarantees that the effects of $x$ and
...
logical regions correspond to a single physical instance, but there are common situations
in which it is desirable to have multiple physical copies of a logical region's
contents in different parts of the machine (e.g., to duplicate and distribute read-only
data) or even to have physical instances of a logical region that temporarily diverge
in their contents (e.g., to keep an old copy of a region to break a write-after-read dependency, or to 
allow independent, local reductions that are later combined).
