...
$y$ on $r$ are the same as if they had executed in
...
guarantees only atomicity of the tasks with respect to $r$: either task $x$
...
\item We define a {\em software out-of-order processor}, or SOOP, for
  ...
  as concurrent and distributed across the machine (Section~\ref{sec:soop}).

\item We describe a {\em deferred execution} model that extracts parallelism
...
over builk synchronous global synchronization primitives such as barriers.
