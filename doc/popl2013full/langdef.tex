...
range over both logical and concrete regions.  Since $M$'s domain is
logical regions, we extend $M$ to constraints with mixed logical and
concrete regions in the standard way by defining
$M \llbracket \rho \rrbracket = \rho$.}
...
replaces all free region names in $T_2$ and particularly $\Omega_1$ by concrete regions, so
...
under $M$, note that $H(\cdot)$ does not depend on the mapping (because it includes only concrete, not logical, regions) so the
