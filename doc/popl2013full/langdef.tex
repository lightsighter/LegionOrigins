...
\finfrule{\begin{array}{l}
\typeenvx e_1 : T_1 \\
\typeenvx[G={\Gamma[T_1/id]}] e_2 : T_2
\end{array}}
...
\infrule[T-Let]{\typeenvx e_1 : T_1 \\ \typeenvx[G={\Gamma[T_1/id]}] e_2 : T_2}{\typeenvx : \text{let}\ id : T_1 \text{in}\ e_2 : T_2} &
...
the use of the following straighforward lemmas:
\begin{lem}
...
We first give an outline of the idea behindthe proof.  For each subexpression of a compound expression,
...
\item Case [E-IfElse-T].
 By assumption, 
$$\opsenv{\text{ if } e_1 \text{ then } e_2 \text{ else } e_3}{v_2}{E}$$ and 
...
For the subexpression $e_1$, 
from [E-IfElse-T] we know $$\opsenv{e_1}{true}{E_1 \dplus E_2}$$
...
$\localconsist{L}{\Gamma}$ and 
$\storeconsist{S}{H}$.
The only type rule that can apply if [T-Newcolor], so can infer that $T = \text{coloring}(r)$.
...
$\localconsist{L}{\Gamma}$ and 
$\storeconsist{S}{H}$.
The only type rule that can apply if [T-Color], so we know $T = \text{coloring}(r)$.
...
we can conlude $\privconsist{E'}{\Phi}$.

\item  Case [E-UpRgn].
By assumption, $$\opsenv{\text{upregion}(e,\oton{r})}{v}{E}$$ and
$$\typeenv{\text{upgregion}(e,\oton{r})}{T}$$ and
$\mapconsist{\Omega}$ and 
$\localconsist{L}{\Gamma}$ and 
$\storeconsist{S}{H}$.
...
$\mapconsist{\Omega}$ and Lemma~\ref{lemma:omegaclosure}, we know there
is some $r_j$ such that $v \in M(r_j)$.
Therefore $\valueconsist{v}{T'@(\oton{r})}$.

\item Case [E-DownRgn].
By assumption, $$\opsenv{\text{downregion}(e,\oton{r})}{v}{E}$$ and
$$\typeenv{\text{downgregion}(e,\oton{r})}{T}$$ and
$\mapconsist{\Omega}$ and 
$\localconsist{L}{\Gamma}$ and 
$\storeconsist{S}{H}$.
...
To show $\valueconsist{v}{T}$ there are two cases.
If $v = M(l)$ then the claim follows from $\valueconsist{l}{T@(\oton{r})}$.
If $v$ is instead some $v'$ of type $H(l)$ the result also follows because $H(l) = \hat{T}$.
Next, $\traceconsist{E \dplus [ read(l,excl,v)]}$ follows immediately from $\traceconsist{E}$ as
any read operation is consistent (the resriction is only on writes and reductions).
...
From [E-Pack] we have   $v' = \langle \langle \otok{k}{\rho}, v \rangle \rangle$.
From [T-Pack] we have $T_1 = \exists r'_1, \ldots r'_n.\ T_2\text{ where }\Omega_1$
and $\Omega_1[r_1/r'_1,\ldots,r_k/r'_k] \subseteq \Omega^*$.  
%
% FIXME  The next sentence can't be made fully formal with the current setup.
%
The result
follows from the definition of value consistency on region relationships.
...
to showing $\valueconsist[M']{L(x)}{\Gamma(x)}$, which must hold becuase $M'$ does 
...
$$S_n = \text{apply}(E_{n-1},\text{apply}(E_{n-2},\ldots \text{apply}(E_1,S) \ldots )$$
From Lemma~\ref{lemma:heapconst:apply} it follows that $\storeconsist{S_n}{H}$.
We also have $\opsenv{e_n}{v_n}{E_n}$ from [E-Call] and
$\typeenv{e_n}{T_n[r_1/r'_1,\ldots,r_k/r'_k]}$ from [T-Call].
By induction, 
...
$valid\_interleave(S, C, E', \oton{E})$
and furthermore
$M' = \{ (r'_1, M(r_1)), \ldots (r'_k, M(r_k)) \}$  and
$L' = \{ (a_1, v_1), \ldots, (a_n, v_n) \}$ and
$S' = \text{apply}(S, E')$ and
$\opsenv[M=M',L=L',S=S',C=C']{e_{n+1}}{v_{n+1}}{E_{n+1}}$.
From [T-Program] we know
$\typeenv[G=\Gamma',O=\emptyset,P=\Phi']{e_{n+1}}{T_r}$,
where $e_{n+1}$ is the body of the function.
To apply the induction hypothesis to $e_{n+1}$ we must
show 
\begin{itemize}
\item $\mapconsist[M']{\emptyset}$,
which is immediate.

\item $\localconsist[M']{L'}{\Gamma'}$ where
$\Gamma'(a_i) = T_i$and $\storeconsist{S'}{H}$.
\end{itemize}
