...
\begin{proof}
The proof is by cases on the structure of the derivation of $e$.
\begin{itemize}
...
$\mapconsist{\Omega}$ and 
$\localconsist{L}{\Gamma}$ and $\storeconsist{S}{H}$.
The only type rule that can apply is [T-MakeType], so $T = \langle T_1, T_2 \rangle$.
...
Finally, from the precedinig steps we can conclude that $\valueconsist{\langle v_1, v_2\rangle}{\langle T_1, T_2 \rangle}$.
By Lemma~\ref{lemma:heapconst:effects1}, $\traceconsist{E_1 \dplus E_2}$, and by Lemma~\ref{lemma:effsound:effects1}
...
\textit{true} replaced by \textit{false}.
