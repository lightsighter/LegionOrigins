...
Listing~\ref{lst:circuit_ex_full} shows a circuit simulation written (with
one exception) in the core Legion language (see
Section~\ref{sec:legioncorefull}).  The input is a graph of circuit
...
Section~\ref{sec:legioncorefull}, they can be easily implemented using a separate 
...
\begin{lstlisting}[float={t},label={lst:circuit_ex_full},caption={Circuit Simulation}]
...
modes is formalized in Section~\ref{sec:coherencefull}.
...
\begin{lstlisting}[float={t},label={lst:circuit_leaf_full},caption={Circuit Leaf Functions}]
