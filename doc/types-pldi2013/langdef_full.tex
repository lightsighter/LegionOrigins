...
\valueconsist[]{\langle \langle \oton{\rho}, v \rangle \rangle}{\text{rr}[\oton{r}]\ \hat T\text{ where }\hat \Omega} & \Leftrightarrow & (\valueconsist[]{v}{\hat T[\rho_1/r_1, \ldots \rho_n/r_n]}) \wedge (\mapconsist[\{ (r_i, \rho_i) \}]{\hat \Omega}) \\
...
%  Combining this with $\mapconsist{\Omega}$ and 
...
For the following proofs, we will need one additional lemma, which follows directly
...
\end{proof}
 ...
 The Legion task scheduler uses Theorem~\ref{thm:hierschedfull} in the
