...
becoming increasingly difficult.  Current programming systems for these
machines have intricate features for describing parallelism.  However,
...
Researchers have previously explored language designs with hierarchical 
decomposition of data to express locality and independence for high 
performance programming. Two recent examples, Sequoia \cite{Fatahalian06} and 
Deterministic Parallel Java (DPJ) \cite{Bocchino09}, each provide a 
mechanism to partition the heap recursively into a tree of collections 
...
(see Section~\ref{sec:related} for more discussion of related work).
...
the same data, providing different views on to that data.

To support reasoning about the structure
of data in applications with dynamically instantiated data structures, 
we present a type system and operational semantics for Legion\cite{Legion12}.  
Legion is a programming model that presents {\em logical regions} as an
...
possess privileges.  
%Privileges are passed to a task by their calling task
%or by creating new logical regions.  
%The Legion programming model 
%mandates a functional restriction: a sub-task can only be passed a subset
%of the privileges possesed by its parent task.  As we describe below,
%our type system relies on a combination of static and dynamic checks to 
%enforce this restriction owing to the first-class nature of logical regions.

Legion's ability to describe dynamically instantiated data structures with
multiple dynamic partitions of data does have an important consequence:
aliasing.  Through multiple partitions
...
One consequence of aliasing is that it significantly hinders 
static analyses for proving properties of parallel programs.  
Approaches to dealing with aliasing in the domain
of parallelism have traditionally bifurcated to extremes: Sequoia and DPJ both disallow aliasing
which enables their static analyses at the coarse granularity of regions;  
approaches like transactional memory\cite{} and thread-level speculation\cite{} perform 
fully dynamic analysis at the granularity of individual heap locations.
Alternatively, the type system we present for Legion leverages a hybrid
approach to handling aliasing that enables a combination of static and dynamic
analyses to prove important properties of Legion programs.

The key observation behind our type system is that dynamic alias analysis
is very easy and relatively cheap when done at the granularity of regions 
instead of individual heap locations.  To that end, our type system supports
...
Another important consequence of aliasing is that it often limits 
parallelism.  Legion currently does not attempt to statically prove any two tasks
parallel, but instead relies on dynamic region aliasing checks to discover
parallelism at runtime.  If regions contain many elements, then these checks are
amortized as we will show in Section~\ref{sec:evaluation}.  In addition
to supporting parallel execution in alias-free tasks, Legion also enables
programmer controlled parallel execution in the presence of aliasing.
In many cases, programmers often want to enable parallel execution 
even in the presence of aliased data.  For example, two tasks 
...
can access aliased data while the task is running.  Our operational semantics
provides a formal framework with which to reason about the interaction of tasks
accessing aliased in different coherence modes.

We prove the soundess of both the privilges system and the coherence system.
We then use these results to prove a third result: if two {\em siblings} (tasks that
