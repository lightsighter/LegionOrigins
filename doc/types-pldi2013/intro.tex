...
in distributed memory parallel programs.  One class of work disallows
potential data aliasing to enable fully static analyses with no
runtime overhead, but limited expressivity. Two recent examples, 
...
aliasing and uses fully dynamic analyses of program data that do not 
limit expressivity.  However, this approach incurs significant runtime overhead and requires 
...
a parallel programming model that efficiently reason about the data
aliasing introduced by multiple, dynamic data partitions. 
...
of {\em aliasing}---distinct regions may have overlapping data.\footnote{Thus
...
programmers to permit relaxed models execution even in cases where tasks in
fact do access the same data.  The programmer may
