...
Our own experience writing high-performance 
...
In this work, we present the design of Legion \cite{Legion12},
...
\item programmers give logical regions {\em privilege} and {\em coherence} properties that
describe how the region's data may be used by computations.
\end{itemize}

While data in Legion is organized in a hierarchy of logical region forests, 
...
possess privileges.  Tasks acquire privileges from their calling task
...
aliasing---distinct regions may have overlapping data.\footnote{This
  is why we use the term {\em logical} regions: language-level regions
  ...
  \cite{Legion12}.}  We believe permitting very flexible aliasing is
necessary, but it is also presents a major challenge. For example, to
detect that tasks can execute in parallel it is sufficient to prove
that they work on disjoint data.
...
To efficiently enforce restrictions that guarantee the safety and
correctness of program executions for uses of first-class regions,
their privilege and coherence properties, and arbitrary region
aliasing, the Legion runtime relies on a combination of static and
dynamic checks.  A key theme that runs through the design is that very
hard static analysis problems (e.g., alias analysis) are often both easy and
inexpensive if they can be done dynamically at the granularity of regions instead of individual
heap locations, while checks that must be done on individual heap locations are only
cheap if done statically.  Explicating and evaluating the design of Legion's
static and dynamic semantics and its implications is the main topic of this paper,
which we present as a series of contributions:
\begin{itemize}
\item We present a type system for {\em Core Legion} programs that statically
  verifies the safety of individual pointer references and region
...
\item We present a parallel operational semantics for Core Legion that
  ...
  actually execute on the hardware.  However, the semantics also
  directly models conflicting memory operations between tasks in cases of relaxed
...
\item We prove the soundness of Legion's privelege system (Section~\ref{sec:soundness}).


\item We use the soundness of privileges to prove the soundness of
...
on the target class of machines;  any form of centralized scheduling would be a bottleneck because of the
...
\item We give experimental evidence for the Legion design.  On three
  ...
  checks is low, showing that a much more expressive and dynamic
