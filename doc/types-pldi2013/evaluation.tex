...
\item Overhead: Does it reduce dynamic checking costs (Section~\ref{subsec:overhead})?
\item Scalability: Can it enable hierarchical scheduling (Section~\ref{subsec:scalability})?
\item Performance: Does the performance increase from relaxed coherence modes 
warrant the increased complexity in the operational semantics (Section~\ref{subsec:relaxedperf})?
\end{itemize}
...
sufficiently expressive to handle real-world applications written in the Legion programming model.
