...
\newcommand{\ruleatx}[5]{
\node[below right] at (#1) {$\infrulez{\begin{array}{l}#4\end{array}}{#5}$};
\node[below] at (#2) {[#3]};
}
...
\ruleat{0.1,2.1}{2.5,2.2}{T-Reduce}
{\begin{array}{l}
...
\axiomat{0.1,2.8}{2.5,2.9}{T-New}
{\typeenvx \text{new }T@r : T@r}

\ruleat{0.1,3.2}{2.5,3.3}{T-UpRgn}
{\begin{array}{l}
...
\ruleat{0.1,3.8}{2.5,3.9}{T-DnRgn}
{\begin{array}{l}
\typeenvx e : T@(r'_1, \ldots r'_k) \\
...
\axiomat{0.1,4.1}{2.5,4.2}{T-NewColor}
{\typeenvx \text{newcolor }r : \text{coloring}(r)}

\ruleat{0.1,4.5}{2.5,4.5}{T-Color}
{\begin{array}{l}
...
\ruleat{0.1,5.0}{2.5,5.1}{T-Partition}
{\begin{array}{l}
...
\ruleat{0.0,6.0}{2.5,6.1}{T-Unpack}
{\begin{array}{l}
T_1 = \exists r'_1, \ldots, r'_n.\ T_2\text{ where }\Omega_1 \\
\typeenvx e_1 : T_1 \\
...
\ruleatx{3.0,0.0}{6.5,0.0}{E-Read}{
...
\ruleatx{3.0,1.0}{6.5,1.1}{E-Write}{
...
\ruleatx{3.0,6.0}{6.5,6.1}{E-Partition}{
...
\ruleatx{3.0,6.5}{6.5,6.6}{E-Pack}{
...
\ruleatx{3.0,7.0}{6.5,7.1}{E-Unpack}{
...
\ruleatx{3.0,7.5}{6.5,7.6}{E-Call}{
...
An important insight in the proof of Theorem~\ref{thm:effects} was that 
it did not rely on the definition of {\em valid\_interleave}.  Weakening 
...
traces will be equivalent to a sequential execution, thereby enabling parallel execution.  To
efficiently implement this requirement at runtime, we require a test that can determine this property prior
to the actual execution of the tasks that create the traces.  We show that a test based on the subtask privileges and current region 
mapping can soundly predict when this property will hold.
...
for regions, which can be performed statically.  The ``different address'' condition is approximated by two
...
once per pair of regions rather than for for every pair of memory operations.  Details on how to perform
this test efficiently at runtime can be found in \cite{Legion12}.
