...
that still illustrates all of the important issues.  
%Types include booleans, integers, tuples, and pointers.  
Pointers are annotated with a list of regions---a non-null pointer must point to a 
...
chosen to preserve a straightforward proof of type soundness.
...
exist for a logical region that provably contains the pointer's region.  To simplify this check, Figure~\ref{fig:closure}
...
Partitioning introduces names for subregions corresponding to each color in the coloring, guaranteeing that each
subregion is disjoint from the others and all are subregions of the original region.

The {\em pack} and {\em unpack} expressions allow regions to be stored in the heap.
...
the (fresh) logical region names when the value is read and unpacked from the heap.
...
% 
Finally, task calls verify the requirement that the privileges held by a called task are a subset of those held
by the caller.  In addition to being necessary for the proof of soundness, this property is critical to
enabling hierarchical and distributed scheduling of tasks.
...
The environment also has a {\em clobber set} $C$.
We will show that parallel Legion programs can be scheduled in
a hierarchical fashion.  We therefore require an
operational semantics that is parallel and hierarchical.  Now
Legion allows limited nondeterminism, specifically because of relaxed coherence on
...
value.  Determing which value is deferred until all potentially-concurrent
...
the location falls within the clobber set.  Additionally, the write
and reduce operations, with their two subexpressions, demonstrate how
...
Our first results shows a well-typed expression accesses the heap in ways consistent with its static privileges.
...
\end{array}
$$
Accesses to different locations cannot affect each other, and reads have no side-effects.  The safety of the third case
