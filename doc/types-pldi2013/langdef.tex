...
Pointer types are annotated with a list of logical regions---a non-null pointer value must point to a 
...
be modified by concurrent evaluations.  Reads of locations in $C$ may be
chosen to return a value other than what was most recently written by the current
expression, as long as a
valid interleaving of the operations with other concurrent operations can be found within
an enclosing expression.
%% While a read from a location not in $C$ is {\em coherent}
...
unknown at partitioning time.  The semantics of newcolor allows extra locations to be placed in every
subregion while
...
a logical region whose coherence has been relaxed (via {\em atomic} or {\em simult}) is added to the
subtask's clobber set $C'$, allowing non-deterministic evaluation within the subtask with respect to
those locations.  All actual accesses to those locations within the resulting memory trace ($E_{n+1}$)
are marked with the corresponding coherence mode, allowing the {\em valid\_interleave} check within
the caller's environment to verify the consistency of the overall memory trace.
...
\item local value consistency, written $\localconsist{L}{\Gamma}$, guarantees local values in $L$ have types consistent with the environment $\Gamma$ (using $M$ to map logical regions in $\Gamma$ to physical regions)
...
The stronger constraints address the coherence of heap accesses;
...
enforced.  The validity of the interleaving of these accesses is determined within the context of some enclosing expression.
\end{itemize}
...
Therefore, an adjacent pair of non-interfering memory operations in a memory trace can be reordered while
preserving the validity of an interleaving.
...
Because testing non-interference of tasks is a pairwise operation, scheduling $n$ tasks
can require ${\cal O}(n^2)$ tests.  Thus, a scheduler that must globally consider all
pairs of tasks will be impractical for large machines and large numbers of tasks.  The
following theorem, however, shows that Legion programs enjoy a locality property that
...
privileges.  Since $e_1$ and $e_2$ are called on the parent task's node, no communication
is required to perform the non-interference
