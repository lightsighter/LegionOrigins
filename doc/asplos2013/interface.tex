...
operations have occurred.  Our interface differs from barriers in other programming
...
when the object is created (line 13).  Reduction-only region instances can 
...
An example using reduction-only regions is shown on lines 35-49 of Listing~\ref{lst:regionapi}.
Two reduction-only regions, {\tt inst1} and {\tt inst2}, are created in different memories to
be used with a reduction operation (lines 40-41).  A physical region of the same class {\tt inst3}
