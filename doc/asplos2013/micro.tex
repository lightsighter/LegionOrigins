...
message become necessary, but starts increasing slightly again at larger node counts.  Higher
...
the number of chains result in improved rates.  Note that the point on each line where this occurs
 is when
the chains per node is greater than total locks, i.e. where the expected number of requests per lock 
per node becomes more than one.  As soon as there are multiple expected requestors for the same lock on
a node, the unfairness property of our implementation is able to reduce the number of lock ownership 
...
All reductions are appended to the list, which is then copied back to the global region.
\end{itemize}
...
case uses a histogram with 256K buckets and each reduction task processes a chunk of 4M entries (i.e.
performs 4M reductions).  The results for the
dense experiment are shown in Figure~\ref{fig:reducdense}.  The sparse case use a histogram with 4M buckets,
but only 64K elements are processed by each task.  Results for the sparse case
...
on a single node (where the reads and writes aren't really remote).
