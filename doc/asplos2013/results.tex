...
real-world applications that use the Legion runtime and by extension the heterogeneous
...
circuit nodes, and update the voltages of all nodes.  In the distribute charge
...
which shows the total number of generational events in the system (see Section~\ref{subsec:eventimpl}).
...
to allowing synchronization in an asynchronous environment, the ability of deferred locks to
migrate is necessary for implementing real applications on distributed machines.
