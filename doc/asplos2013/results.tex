...
{\em Circuit} is simulation of an electric circuit that is described as an
...
be difficult for long-running applications.  The {\em live events} line
...
be able to recover the storage associated with the event.  Such an
algorithm would reduce the storage needed by over 10X at the cost of
computation and communication overhead for performing the reference 
counting.  As discussed in Section~\ref{subsec:eventimpl}, our implementation
requires storage that grows with the number of {\em untriggered events}, a number
...
which is larger than the peak number of untriggered events because of the
decision to always create an event owned by the node from which the request
was made.  Despite this inefficiency, our implementation uses 5X less storage
