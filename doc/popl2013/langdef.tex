...
\finfrule{
\begin{array}{l}
\Gamma(id) = \forall \otok{k}{r'}.(\oton{T}),\Phi_i, Q \rightarrow T_r \\
...
\begin{array}{lll}
apply(S, [], C) & = & S \\
apply(S, E \dplus [ read(l, c, v) ]) & = & apply(S, E, C) \\
apply(S, E \dplus [ write(l, c, v) ]) & = & apply(S, E, C)[v/l] \\
apply(S, E \dplus [ reduce_{id}(l, c, v) ]) & = & apply(S, E, C)[id(S[l], v)/l] \\
...
valid\_interleave(S, C, E' \dplus [ read(l, c, v) ] \dplus E'', \oton{E}) & \rightarrow & l \in C \vee S'(l) = v, \text{ where } S' = apply(S, E', C)
\end{array}
$}
...
\end{array}}
{\opsenvx \text{write}(e_1, e_2) \mapsto true, E_1 \dplus E_2 \dplus [ write(l, excl, v) ]} \\
...
\opsenvx[S=S'] e_2 \mapsto v, E_2 \\
\end{array}}
{\opsenvx \text{reduce}(id, e_1, e_2) \mapsto true, E_1 \dplus E_2 \dplus [ reduce_{id}(l, excl, v) ]} \\
...
valid\_interleave(S, C, E', E_1, E_2)
\end{array}}
{\opsenvx \text{if }e_1\text{ then }e_2\text{ else }e_3 \mapsto v_2, E'} \\
...
\end{array}}
{\opsenvx \text{if }e_1\text{ then }e_2\text{ else }e_3 \mapsto v_3, E'} \\
...
S' = \text{apply}(S, E', C) \\
task id[\otok{k}{r'}](f_1 : T_1, \ldots, f_n : T_n), \Phi_i, Q : T_r = e_{n+1} \\
