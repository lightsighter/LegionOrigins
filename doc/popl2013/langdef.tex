...
\label{thm:effects}
\rm
If $\typeenv{e}{T}$ and $\opsenv{e}{v}{E}$  and $M \sim \Omega$, $L \sim_H \Gamma$ and $S \sim H$,
then
$v \sim_H M \llbracket T \rrbracket$, $E \sim H$ and $M \vdash E : \Phi$.
\end{thm}
...
\item[$E \sim H$] The type consistency of the values in an expression's memory
trace follows from the following lemma applied to the traces of subexpressions:
if $E_1 \sim H$ and $E_2 \sim H$ and $any\_interleave(E', E_1, E_2)$, then $E' \sim H$.
