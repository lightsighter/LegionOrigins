...
{\sc E-Call} uses the original $C$ instead of $C'$.  Note that this change
...
ordering of subexpression evaluation, but the Legion core language does not
guarantee parallel execution in any case, so the evaluation of such an 
...
\rm
Let $\oton{e}$ be expressions such that $\opsenv[S={S_{i-1}}]{e_i}{v_i}{E_i}$, 
