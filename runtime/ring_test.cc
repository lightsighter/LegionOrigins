...
void potato_launcher(const void * args, size_t arglen, Processor p)
{
	...
	for (int i=0; i<NUM_POTATOES; i++)
	{
		...
		counter_instance.write(potato.lap_count_location,NUM_TRIPS);
...
void hot_potatoer(const void * args, size_t arglen, Processor p)
{
	...
	if (potato.hops_left == 0)
	{
		...
		unsigned trips = local_inst.read(potato.lap_count_location);
		if (trips == 0)
		{
		...
		else
		{
			...
			// Decrement the count
			local_inst.write(potato.lap_count_location,trips-1);
			Processor target = neighbor;
