...
#for (  [2, 1, 12], [3, 1, 12], [4, 1, 12]) {
	...
	my $cmd = "gasnetrun_ibv -n ${nn} ./cktgpu2 -ll:gpu ${ng} -level 1 -cat app -npp 25000 -wpp 100000 -ll:fsize 3072 -ll:zsize 1536 -ll:csize 8192 -ll:gsize 2000 -l 5 -p ${np} > $outfile";
	print "$cmd\n";
	for my $tries (1..5) {
	    my $res = system($cmd);
	    print "result = $res\n";
